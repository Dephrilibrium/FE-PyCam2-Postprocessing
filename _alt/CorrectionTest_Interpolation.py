...
from PMPLib.Correction.Interpolation import InterpolateLuminosityVector
...
imags = dict()
imags[100000] = dict()
imags[100000]["uint8"] = [cv.imread(r"C:\Users\ham38517\Desktop\05 PiCam\_Auswertungs-Scripte\_TestImages\Dev101_HQCam-0937_ss=100000_0003.jpg", cv.IMREAD_GRAYSCALE)]   # UHD
XY = [(2061, 1492)]
...
sideLen = 60
sideHei = 45
x1, y1, x2, y2 = GetXYArea_XYX2Y2(XYTuple=XY[0], pxWidth=sideLen, pxHeight=sideHei)
...
center, r, oeCoord = CenterAndCoordOfBrightMin(tImg, BMin=bRepl, Dilate=5, Erode=5, ShowImg=True)
# !!! center[0] = x ; center[1] = y
# !!! oeCoord[0] = y; oeCoord[1] = x !!!

divVecPx = list()
divVecSum = list()

rImg = tImg.copy().astype(np.uint16)
estOE = 1 + (np.where(tImg >= bRepl)[0].__len__() / np.where(tImg >= 25)[0].__len__())
# estOE = 354/254
_iter = 0
print("Divs are in: BrightCorrected / BrightOriginal")
replaceCollection = dict()
replaceCollection["coord"] = list()
replaceCollection["value"] = list()
for coord in zip(oeCoord[0], oeCoord[1]): # correct xy-order: y <-> x
  plt.close("all")
  _iter += 1
  bVec, iCoord = GrabLuminosityVector4Coord(tImg=tImg, center=center, coord=coord, overlap=-1, flip=True, showImg=True)
  # bVec, iCoord = GrabLuminosityVector4Coord(tImg=tImg, center=center, coord=coord, overlap=int(r/4), flip=True, showImg=True)
  # bVec, iCoord = GrabLuminosityVector4Coord(tImg=tImg, center=center, coord=coord, overlap=0, flip=True, showImg=True)
  ipVec = InterpolateLuminosityVector(bVec=bVec, bAdj=estOE, sMin=20, bMax=bRepl)
  plt.plot(iCoord, ipVec["y"][iCoord], "r2", markersize=10, label="Replace-Px")
  # rImg[coord] = ipVec["y"][iCoord]
  replaceCollection["coord"].append(coord)
  replaceCollection["value"].append(ipVec["y"][iCoord])
  divVecPx.append(ipVec["y"][iCoord] / bVec["y"][iCoord])
  divVecSum.append(np.sum(ipVec["y"]) / np.sum(bVec["y"]))
  print(str.format("Pixel-Replace [{:3d}] @ {}:".ljust(30) + "{} -> {}".ljust(15) + "DivPx: {:.5f}".ljust(20) + "DivSum: {:.5f}", _iter, coord, tImg[coord], replaceCollection["value"][-1], divVecPx[-1], divVecSum[-1]))
...
for (coord, value) in zip(replaceCollection["coord"], replaceCollection["value"]):
  rImg[coord] = value
