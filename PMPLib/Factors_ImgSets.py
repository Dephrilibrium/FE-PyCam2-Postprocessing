...
def __BuildImgSubSet__(ImgCollection, XYCollection, pxSidelen, AddPxSidelen, OverexposedBright, CleanMask):
  subSet = dict()
  subSet["Full"] = dict()
...
def BuildFactorImgSets(ssData, ImgKey, pxSidelen, AddPxSidelen, OverexposedValue):
  ...
  for _iBase in range(len(_ssKeys) -1):
    ...
    bImgs[_bKey] = __BuildImgSubSet__(ImgCollection=pickle.loads(pickle.dumps(ssData[_bKey]["Images"][ImgKey])), XYCollection=ssData[_bKey]["Circles"]["XYKeys"], pxSidelen=pxSidelen, AddPxSidelen=AddPxSidelen, OverexposedBright=OverexposedValue, CleanMask=None)
    ...
    for _iDiv in range(_iBase + 1, len(_ssKeys)):
      ...
      bImgs[_bKey]["Div"][_dKey] = __BuildImgSubSet__(ImgCollection=pickle.loads(pickle.dumps(ssData[_dKey]["Images"][ImgKey])), XYCollection=ssData[_dKey]["Circles"]["XYKeys"], pxSidelen=pxSidelen, AddPxSidelen=AddPxSidelen, OverexposedBright=OverexposedValue, CleanMask=bImgs[_bKey]["Full"]["OverexposedMask"])
