...
class PiMageOptions():
  ...
  def Save(self, saveDir:str, fName:str):
    ...
    _output += str.format("\n[Spot-detection]\n")
    _output += str.format("opt.SpotDetect_Dilate"                           .ljust(_fill) + "={}\n", self.SpotDetect_Dilate     )                         # Detected image-contours (on thresh-images) are extended by n pixel-rows (entire circumfence) to close small gaps between a splitted spot
    _output += str.format("opt.SpotDetect_Erode"                            .ljust(_fill) + "={}\n", self.SpotDetect_Erode      )                         # The dilated image-contours are reduced by n pixel-rows (entire circumfence) (if erode=dilate the resulting spot should be the same as initially but whitout missing pixels within)
    _output += str.format("opt.SpotDetect_pxMinRadius"                      .ljust(_fill) + "={}\n", self.SpotDetect_pxMinRadius)                         # Minimum radius for a valid spot: pxMinRadius <= r <= pxMaxRadius; Used to avoid artifacts detected as spots
    _output += str.format("opt.SpotDetect_pxMaxRadius"                      .ljust(_fill) + "={}\n", self.SpotDetect_pxMaxRadius)                         # Maximum radius for a valid spot: pxMinRadius <= r <= pxMaxRadius; Used to avoid the detection of spots bigger than being estimated



    _output += str.format("\n[Spot-Draw on Images]\n")
    _output += str.format("opt.CircleDraw_pxRadius"                         .ljust(_fill) + "={}\n", self.CircleDraw_pxRadius   )                         # Draws a circle using pxRadius to visualize the detected spots on the "circle-draw-images"
    _output += str.format("opt.CircleDraw_AddPxRadius"                      .ljust(_fill) + "={}\n", self.CircleDraw_AddPxRadius)                         # When enabled, the circles detected spot-radius is added to pxRadius
    _output += str.format("opt.CircleDraw_pxThickness"                      .ljust(_fill) + "={}\n", self.CircleDraw_pxThickness)                         # Determine the line-thickness of the drawn circles around of each spot respectively each xyKey in [px]


    _output += str.format("\n[Brightness-Extraction]\n")
    _output += str.format("opt.SesExtraction_pxSideLen"                     .ljust(_fill) + "={}\n", self.SesExtraction_pxSideLen   )                     # Sidelength of a square around the circle center from which the circle-brightness is extracted
    _output += str.format("opt.SesExtraction_AddPxSideLen"                  .ljust(_fill) + "={}\n", self.SesExtraction_AddPxSideLen)                     # When enabled, the circles radius is added to bDetect_pxSideLen



    _output += str.format("\n[XY-Keys]\n")
    _output += str.format("opt.XYKeys_pxCollectRadius"                      .ljust(_fill) + "={}\n", self.XYKeys_pxCollectRadius   )                      # Valid radius around a xy-coordinate a detected image-spot is collected under, when located within this area.
    _output += str.format("opt.XYKeys_AddPxCollectRadius"                   .ljust(_fill) + "={}\n", self.XYKeys_AddPxCollectRadius)                      # When enabled, the image-spots radius is added to SpotDetect_pxMaxRadius
    _output += str.format("opt.XYKeys_FollowSpots"                          .ljust(_fill) + "={}\n", self.XYKeys_FollowSpots       )                      # When enabled, the xy-key follows the related circle-centers by meaning them
    _output += str.format("opt.XYKeys_pxCorrectionRadius"                   .ljust(_fill) + "={}\n", self.XYKeys_pxCorrectionRadius)                      # For each SS a set of XY-Key-Pairs can be found. To assign them together, this radius is used as a tolerance and is then re-attached to ssData.
    _output += str.format("opt.XYKeys_CrossCheckKeys"                       .ljust(_fill) + "={}\n", self.XYKeys_CrossCheckKeys)                          # This option defines if the XYKeys are iterated again and combined if they are in range of each other (range = opt.XYKeys_pxCorrectionRadius)



    _output += str.format("\n[XY-Key Sorting]\n")
    _output += str.format("opt.XYKeySort_Rowdistance"                       .ljust(_fill) + "={}\n", self.XYKeySort_Rowdistance)                          # Each leftmost spot of a row does a horizontal raycast to the right. XYKeySort_Rowdistance (in [px]) defines the vertical distance to that ray, in which a spot needs to be located to count as part of the row.



    _output += str.format("\n[Dump, Save & Datacopy]\n")
    _output += str.format("opt.PngDump_16bitGrayScale"                      .ljust(_fill) + "={}\n", self.PngDump_16bitGrayScale                   )      # Dump imagecollection of the raw 16-bit images as PNGs                                                   (highest SS only)
    _output += str.format("opt.PngDump_8bitThreshhold"                      .ljust(_fill) + "={}\n", self.PngDump_8bitThreshhold                   )      # Dump imagecollection of the raw threshhold images as PNGs                                               (highest SS only)
    _output += str.format("opt.PngDump_8bitCircleDetect"                    .ljust(_fill) + "={}\n", self.PngDump_8bitCircleDetect                 )      # Dump imagecollection of the dilated and eroded threshhold images, used for the circle-detection as PNGs (highest SS only)
    _output += str.format("opt.PngDump_8bitCircleDrawAroundEachDetection"   .ljust(_fill) + "={}\n", self.PngDump_8bitCircleDrawAroundEachDetection)      # Dump imagecollection of the 8-bit images with cirlces drawn around each detected spot as PNGs           (highest SS only)
    _output += str.format("opt.PngDump_8bitXYKeyDrawAroundEachXYKey"        .ljust(_fill) + "={}\n", self.PngDump_8bitXYKeyDrawAroundEachXYKey     )      # Dump imagecollection of the 8-bit images with cirlces drawn around each XYKey as PNGs                   (highest SS only)

    _output += str.format("opt.PklDump_imgContainer"                        .ljust(_fill) + "={}\n", self.PklDump_imgContainer                     )      # Dump the entire image-container as pickle-binary (all images incl. subarea-images -> HUGE filesize)
    _output += str.format("opt.PklDump_cirContainer"                        .ljust(_fill) + "={}\n", self.PklDump_cirContainer                     )      # Dump the circle-container as pickle-binary (all circle data)
    _output += str.format("opt.PklDump_pcoContainer"                        .ljust(_fill) + "={}\n", self.PklDump_pcoContainer                     )      # Dump the PixelCount- and Overexposureinfo-container as pickle-binary
    _output += str.format("opt.PklDump_sesContainer"                        .ljust(_fill) + "={}\n", self.PklDump_sesContainer                     )      # Dump the SEnsorSignal-container as pickle-binary
    _output += str.format("opt.PklDump_mssContainer"                        .ljust(_fill) + "={}\n", self.PklDump_mssContainer                     )      # Dump the MergedSensorSignal-container as pickle-binary (combination of the sensor signals on the different SS-images)

    _output += str.format("opt.Copy_FEMDAQData"                       .ljust(_fill) + "={}\n", self.Copy_FEMDAQData         )           # Creates copies of the relevant FEMDAQ-data (if FEMDAQ was used as measurement-tool!)
