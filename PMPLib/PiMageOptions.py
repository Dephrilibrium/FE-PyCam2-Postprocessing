...
class PiMageOptions():
  ...
  def Save(self, saveDir:str, fName:str):
    ...
    _output += str.format("opt.CircleDetect_pxMinRadius"              .ljust(_fill) + "={}", self.CircleDetect_pxMinRadius)           # Minimum radius for a valid spot: pxMinRadius <= r <= pxMaxRadius; Used to avoid artifacts detected as spots
    _output += str.format("opt.CircleDetect_pxMaxRadius"              .ljust(_fill) + "={}", self.CircleDetect_pxMaxRadius)           # Maximum radius for a valid spot: pxMinRadius <= r <= pxMaxRadius; Used to avoid the detection of spots bigger than being estimated
    ...
    _output += str.format("opt.BrightExtract_pxSideLen"               .ljust(_fill) + "={}", self.BrightExtract_pxSideLen   )         # Sidelength of a square around the circle center from which the circle-brightness is extracted
    _output += str.format("opt.BrightExtract_AddPxSideLen"            .ljust(_fill) + "={}", self.BrightExtract_AddPxSideLen)         # When enabled, the circles radius is added to bDetect_pxSideLen
    ...
    _output += str.format("opt.XYKeys_AddPxCollectRadius"             .ljust(_fill) + "={}", self.XYKeys_AddPxCollectRadius)          # When enabled, the image-spots radius is added to CircleDetect_pxMaxRadius
    ...
    _output += str.format("opt.PngDump_8bitCircleDraw"                .ljust(_fill) + "={}", self.PngDump_8bitCircleDraw  )           # Dump imagecollection of the 8-bit images with cirlces drawn around each detected spot as PNGs           (highest SS only)
