...
def PlotLegend(fig, axes=None, loc="upper right", ncol=1, LabelColor="linecolor"):
    lines = []
    labels = []
    if axes==None:
        axes = fig.axes
...
def ShowMajorMinorY(axis, useLogLocator=True):
    for ax in axis:
        ax.grid(visible=True, which="both", axis="both", color="#CCCCCC", linestyle="-", linewidth=0.5)
